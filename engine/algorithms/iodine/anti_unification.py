...
def anti_unify(list1, list2, index1, index2, worktable, num_holes=0, max_hole_size=0):
    """
    ...
    Arguments:
        ...
        max_hole_size {int} -- maximum number of holes accumulated in the tree
    """
    ...
    if list1[index1] == list2[index2]:  # if true:
        ...
        if not list1[index1].child_indices and not list2[index2].child_indices:
            return PatternNode(list1[index1], list2[index2], list1[index1].value), num_holes, max_hole_size
            ...
            pat_max = max(len(node1.getAllChildren()), len(node2.getAllChildren()))
            if max_hole_size < pat_max:
                max_hole_size = pat_max
            return pat, num_holes, max_hole_size
            ...
            for cii, ci1 in enumerate(list1[index1].child_indices):
                ...
                subtree, num_holes, max_hole_size = anti_unify(list1, list2, ci1, ci2, worktable, num_holes)
                # associate with p
                p.add_child(subtree)
            return p, num_holes, max_hole_size
    else:  # if false:
        return PatternNode(list1[index1], list2[index2]), num_holes, max_hole_size
